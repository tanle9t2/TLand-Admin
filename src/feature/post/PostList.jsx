...
import useAcceptPost from './useAcceptPost';
import toast from 'react-hot-toast';
import useRejectPost from './useRejectPost';


export default function PostList({ isLoading, post, rowCount }) {
  ...
  function handleOnClickAccept(postId) {
    acceptPost({ postId }, {
      onSuccess: () => toast.success("Bài viết đã được duyệt"),
  ...
  const handleRowDelete = React.useCallback(
    (post) => async () => {
      const confirmed = await dialogs.confirm(
        `Bạn thật sự muốn từ chối bài đăng ${post.title}?`,
        {
  ...
  const columns = React.useMemo(
      ...
      },
      {
        ...
        getActions: ({ row }) => [
          <GridActionsCellItem
            ...
            label="Delete"
            onClick={() => handleOnClickAccept(row.id)}
          />,
          <GridActionsCellItem
            key="delete-item"
            icon={<TiDeleteOutline size={18} />}
            label="Delete"
    ...
    [handleOnClickAccept, handleRowDelete],
  );
